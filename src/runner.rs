...
use crate::usage::{Usage, UsageCalculator, calculate_costs};
...
impl Metrics {
    ...
    pub fn get_and_reset(&self) -> (usize, Histogram<u64>) {
        let mut inner = self.inner.lock().unwrap();
...
pub fn print_usage(latest_usage: &Usage) {
    ...
    let total_cost = latest_usage.cost_estimate.total_dpus.total + latest_usage.cost_estimate.latest_storage.gb_month;
    
    println!("\n{:=^70}", " Usage & Cost ");
    println!("{:<15} {:>15} {:>15}", "", "Usage", "Cost");
    println!("{:-<15} {:-^15} {:-^15}", "", "", "");
    println!("{:<15} {:>15.2} {:>15}", "Total DPUs:", latest_usage.dpu_metrics.total, format!("${:.2}", latest_usage.cost_estimate.total_dpus.total));
    println!("{:<15} {:>15.2} {:>15}", "  Compute:", latest_usage.dpu_metrics.compute, format!("${:.2}", latest_usage.cost_estimate.total_dpus.compute));
    println!("{:<15} {:>15.2} {:>15}", "  Read:", latest_usage.dpu_metrics.read, format!("${:.2}", latest_usage.cost_estimate.total_dpus.read));
    println!("{:<15} {:>15.2} {:>15}", "  Write:", latest_usage.dpu_metrics.write, format!("${:.2}", latest_usage.cost_estimate.total_dpus.write));
    println!("{:<15} {:>15} {:>15}", "Storage:", storage.to_string(), format!("${:.2}", latest_usage.cost_estimate.latest_storage.gb_month));
    println!("{:═<15} {:═^15} {:═^15}", "", "", "");
    println!("{:<15} {:>15} {:>15}", "TOTAL COST:", "", format!("${:.2}", total_cost));
    println!("{:=^70}", "");
}
...
pub fn print_usage_and_diff(latest_usage: &Usage, usage_diff: &Usage) {
    ...
    println!("\n{:=^90}", " Usage & Cost ");
    println!("{:<15} {:>15} {:>15} {:>15} {:>15}", "", "Usage", "Delta", "Cost", "Delta");
    println!("{:-<15} {:-^15} {:-^15} {:-^15} {:-^15}", "", "", "", "", "");
    println!("{:<15} {:>15.2} {:>15} {:>15} {:>15}", 
        "Total DPUs:", 
    ...
    println!("{:<15} {:>15} {:>15} {:>15} {:>15}", 
        ...
        format!("(+${:.2})", usage_diff.cost_estimate.latest_storage.gb_month)
    );
    println!("{:═<15} {:═^15} {:═^15} {:═^15} {:═^15}", "", "", "", "", "");
    println!("{:<15} {:>15} {:>15} {:>15} {:>15}", 
        "TOTAL COST:", 
