...
impl Ui {
    ...
    pub fn on_before_setup(&mut self) {
        let SetupState { multi, .. } = &self.state.setup;
        multi.set_draw_target(ProgressDrawTarget::stderr());
    }

    pub fn on_after_setup(&mut self) {
        let SetupState { multi, pbs } = &self.state.setup;
        let mut pbs = pbs.lock().unwrap();
        for (_, pb) in pbs.drain(..) {
            pb.disable_steady_tick(); // defensive
            pb.finish_and_clear();
        }
        _ = multi.clear();
    }
...
struct SetupState {
    multi: MultiProgress,
    pbs: Mutex<IndexMap<String, ProgressBar>>,
}

impl Default for SetupState {
    fn default() -> Self {
        let multi = MultiProgress::with_draw_target(ProgressDrawTarget::hidden());
        let pbs = Mutex::new(IndexMap::new());
        Self { multi, pbs }
    }
}

impl SetupState {
    fn get_or_create(&self, name: impl Into<String>) -> ProgressBar {
        let mut map = self.pbs.lock().expect("never poisoned");
    ...
    while let Some(event) = rx.recv().await {
        match event {
            ...
            Message::TableDropping(t) => {
                let pb = &state.setup.get_or_create(&t);
